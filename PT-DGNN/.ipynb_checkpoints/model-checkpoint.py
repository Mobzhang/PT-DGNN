...
class GNN(nn.Module):
    ...
    def get_loss_sage(self, embd, node_positive_pairs, node_negative_pairs, device):
        #computer unsupervised loss

        Q = 10
        assert len(node_positive_pairs) == len(node_negative_pairs)
        nodes_score = []
        for node in node_positive_pairs:
            pps = node_positive_pairs[node]
            nps = node_negative_pairs[node]
            if len(pps) == 0 or len(nps) == 0:
                continue
            indexs = [list(x) for x in zip(*nps)]
            node_indexs = torch.LongTensor([x for x in indexs[0]]).to(device)
            negb_indexs = torch.LongTensor([x for x in indexs[1]]).to(device)
#             print('node_indexs:', node_indexs)
#             print('negb_indexs:', negb_indexs)
#             print('embd device', embd.device)
#             print('negb device', negb_indexs.device)
            neg_score = F.cosine_similarity(embd[node_indexs], embd[negb_indexs])
            neg_score = Q * torch.mean(torch.log(torch.sigmoid(-neg_score)), 0)
#             print('neg_score:', neg_score.device)

            indexs = [list(x) for x in zip(*pps)]
            node_indexs = torch.LongTensor([x for x in indexs[0]])
            negb_indexs = torch.LongTensor([x for x in indexs[1]])
            pos_score = F.cosine_similarity(embd[node_indexs], embd[negb_indexs])
