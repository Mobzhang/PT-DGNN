...
def sample_subgraph(graph, time_range, sampled_depth = 2, sampled_number = 8, inp = None, feature_extractor = feature_OAG, ist = 1):
    '''
...
#                 print(relation_type)
#                 if relation_type == 'def': ## !!!!!!!!!!!!
#                     continue
#                 if relation_type == 'time': ## !!!!!!!!!!!!
#                     continue
                if relation_type == 'self' or target_id not in tes[relation_type]:
                    continue
                adl = tes[relation_type][target_id]   ### adl是节点target_id的所有邻居
#                 print(adl)
                if len(adl) < sampled_number:        ### 对邻居进行随机选择sampled_number数量的邻居
                    sampled_ids = list(adl.keys())
                else:
                    sampled_ids = np.random.choice(list(adl.keys()), sampled_number, replace = False)
#                 print(sampled_ids)
                for source_id in sampled_ids:       ### 对候选邻居更新他们被采样的概率（概率等于候选邻居总和的倒数）
                    source_time = adl[source_id]
#                     print('adl', adl, 'source_id', source_id, 'source_time', source_time)
                    if source_time == None:
...
#                     budget[source_type][source_id][0] += source_time
                    budget[source_type][source_id][1] = source_time
#                     print(budget[source_type][source_id][0], budget[source_type][source_id][1])

    '''
        First adding the sampled nodes then updating budget.
    '''
#     print(inp)
#     print(layer_data['def'])
    for _type in inp:
        for _id, _time in inp[_type]:
#             print(len(layer_data[_type]))
            layer_data[_type][_id] = [len(layer_data[_type]), _time]    
#             print("_type: ", _type, "_id: ", _id, "_time: ", _time, "len: ", len(layer_data[_type]))
#     print(graph.edge_list['def']['def']) 
#     print(layer_data['def'])
    for _type in inp:
...
#             print('len(keys):', len(keys), 'keys:', keys, 'len(sampled_keys)', len(sampled_keys), 'sampled_keys', sampled_keys)
            '''
                First adding the sampled nodes then updating budget.
            '''
                ...
                for target_key in tld:
                    ...
                    for source_key in tesr[target_key]:
                        ...
                        if source_key in sld:
                            ...
                            if ist:
                                edge_list[target_type][source_type][relation_type] += [[target_ser, source_ser, tes['time'][target_key][source_key]]]
                            else:
                                edge_list[target_type][source_type][relation_type] += [[target_ser, source_ser]]                            
    return feature, times, edge_list, indxs, texts


def to_torch(feature, time, edge_list, graph, num_neg):
    '''
...
#                 print(relation_type)
                if(relation_type in {'time'}):  ##改动
#                     print(edge_list[target_type][source_type][relation_type])
                    for ii,(ti,si,time) in enumerate(edge_list[target_type][source_type][relation_type]):
                        tid, sid = int(ti + node_dict[target_type][0]), int(si + node_dict[source_type][0])
                        time_index +=[[sid,tid,time]]
                else:
                    for ii, (ti, si) in enumerate(edge_list[target_type][source_type][relation_type]):
                        ...
                        '''
                        edge_time  += [node_time[int(tid)] - node_time[int(sid)] + 120]

#     if 'time' in edge_list['def']['def']:
#         node_positive_pairs, node_negative_pairs = node_pairs_sample(time_index, num_neg) 
#     else:
#         node_positive_pairs, node_negative_pairs = node_pairs_sample(edge_index, num_neg)
    node_positive_pairs, node_negative_pairs = node_pairs_sample(edge_index, num_neg) 
    ...
    return node_feature, node_type, edge_time, edge_index, edge_type, node_positive_pairs, node_negative_pairs, node_dict, edge_dict

def get_positive_pairs(G, nodes):
    node_positive_pairs = {}
    for node in nodes:
        if G.degree(node) == 0:
            continue
...
#                 time = 0
#                 for i in neighs:
# #                     print(G[node][i]['weight'])
#                     time += G[node][i]['weight']
#                 for i in neighs:
#                     prob.append(G[node][i]['weight']/time)
# #                 print("neighbors",neighs)
# #                 print("prob",prob)
#                 next_node = np.random.choice(neighs, p=prob)
                next_node = np.random.choice(neighs)
                if next_node != node:
                    cur_pairs.append((node, next_node))
                curr_node = next_node
        node_positive_pairs[node] = cur_pairs
    return node_positive_pairs
def get_negative_pairs(G, nodes, num_neg):
    node_negative_pairs = {}
    for node in nodes:
        neighbors = set([node])
        frontier = set([node])
...
#                 neighs = [n for n in G.neighbors(node)]
                neighs = [n for n in G.neighbors(outer)]
                current |= set(neighs)
            frontier = current - neighbors
            neighbors |= current
        far_nodes = set(G.nodes()) - neighbors
        neg_samples = random.sample(far_nodes, num_neg) if num_neg < len(far_nodes) else far_nodes
        node_negative_pairs[node] = [(node, neg_node) for neg_node in neg_samples]
    return node_negative_pairs
def node_pairs_sample(edge_index, num_neg):
    '''
...
#     G.add_weighted_edges_from(edge_index)
    ##new!!!!!!!!!!!!!!
    ##new!!!!!!!!!!!
#     nodes = random.sample(list(G.nodes()), 256)
    nodes = list(G.nodes())
    node_positive_pairs = get_positive_pairs(G, nodes)
    node_negative_pairs = get_negative_pairs(G, nodes, num_neg)
    return node_positive_pairs, node_negative_pairs
